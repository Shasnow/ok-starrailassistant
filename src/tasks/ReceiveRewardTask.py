...
class ReceiveRewardTask(BaseTask):
    ...
    def daily_reward(self):
        self.info_set(self.name, "领取每日实训奖励")
        for i in range(30):
        ...
        if len(count) == 0:
            self.info_set(self.name, "没有可领取的奖励")
            return
        count = count[0].name
        self.info_set(self.name, "活跃度：" + count)
        match count:
            case "100":
    ...
    def nameless_honor(self):
        self.info_set(self.name, "领取无名勋礼")
        for i in range(30):
        ...
        self.send_key('esc', down_time=0.1)
        self.info_set(self.name, '完成：领取无名勋礼')

    def esc_page_locate(self):
        self.info_set(self.name, "定位ESC页面")
        for i in range(30):
            if not self.ocr(0.67, 0.23, 0.72, 0.26, match="开拓等级", log=True):
                self.send_key('esc', down_time=0.1)
                self.sleep(2)
            else:
                self.info_set(self.name, "已定位到ESC页面")
                return True
        else:
            self.log_error('Time out')
            return False
